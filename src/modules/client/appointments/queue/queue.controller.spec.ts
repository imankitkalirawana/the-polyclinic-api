...
describe('QueueController', () => {
  ...
  describe('create', () => {
    it('should create a new queue with CASH payment', async () => {
      ...
      const result = await controller.create(mockStandardParams as any, createDto);
    ...
    it('should use existing queue when queueId is provided', async () => {
      ...
      expect(result).toEqual(mockQueue);
      expect(mockQueueService.findOne).toHaveBeenCalledWith('existing-queue-123');
    ...
    it('should create payment for RAZORPAY payment mode', async () => {
      ...
      (mockQueueService.create as jest.Mock).mockResolvedValue(mockQueue);
      (mockQueueService.createPayment as jest.Mock).mockResolvedValue(mockPayment);
  ...
  describe('verifyPayment', () => {
    it('should verify payment', async () => {
      const mockQueue = createMockFormattedQueue({ status: QueueStatus.BOOKED });
      (mockQueueService.verifyPayment as jest.Mock).mockResolvedValue(mockQueue);
  ...
  describe('getQueueForDoctor', () => {
    ...
    it('should pass queue id and date when provided', async () => {
      ...
      await controller.getQueueForDoctor('doctor-123', 'queue-123', '2026-01-17');
  ...
  describe('getActivityLogs', () => {
    it('should return activity logs for queue', async () => {
      ...
      (mockQueueService.getActivityLogs as jest.Mock).mockResolvedValue(mockLogs);

      const result = await controller.getActivityLogs('queue-123');

      expect(result).toEqual(mockLogs);
      expect(mockQueueService.getActivityLogs).toHaveBeenCalledWith('queue-123');
    });
  });
  ...
  describe('update', () => {
    it('should update queue', async () => {
      ...
      expect(mockQueueService.update).toHaveBeenCalledWith('queue-123', updateDto);
    });
  });
  ...
  describe('callQueue', () => {
    it('should call queue and set message', async () => {
      const mockQueue = createMockFormattedQueue({ status: QueueStatus.CALLED });
      (mockQueueService.callQueue as jest.Mock).mockResolvedValue(mockQueue);

      const result = await controller.callQueue(mockStandardParams as any, 'queue-123');
  ...
  describe('clockIn', () => {
    it('should clock in and set message', async () => {
      ...
      const result = await controller.clockIn(mockStandardParams as any, 'queue-123');
  ...
  describe('skipQueue', () => {
    it('should skip queue and set message', async () => {
      const mockQueue = createMockFormattedQueue({ status: QueueStatus.SKIPPED });
      (mockQueueService.skipQueue as jest.Mock).mockResolvedValue(mockQueue);

      const result = await controller.skipQueue(mockStandardParams as any, 'queue-123');
  ...
  describe('completeAppointmentQueue', () => {
    it('should complete appointment and set message', async () => {
      const mockQueue = createMockFormattedQueue({ status: QueueStatus.COMPLETED });
      (mockQueueService.completeAppointmentQueue as jest.Mock).mockResolvedValue(
        mockQueue,
      );
  ...
  describe('appointmentReceiptPdf', () => {
    it('should return PDF response with correct headers', async () => {
      ...
      expect(mockQueueService.appointmentReceiptPdf).toHaveBeenCalledWith('queue-123');
